<template>
  <div class="detail-content">
    <el-scrollbar @scroll="scrollHandle" ref="detail-content">
  ...
  setup(props: any, { emit }: any) {
    ...
    function scrollHandle({ scrollTop }: any) {
      ...
      if (
        ...
        if (proxy.$refs["pointsBox"] instanceof Array) {
          let clientHeight = 0;
    ...
    function scrollToIndex(index: number) {
      if (!proxy.$refs["detail-content"]) return;
      if (!proxy.$refs["pointsBox"]) return;
      if (!(proxy.$refs["pointsBox"] instanceof Array)) return;
      let clientHeight = 0;
        ...
        if (+props.matchInfo.is_finish) {
          ...
          for (const play of list) {
            if (!(+play.is_del || +play.is_finish || +play.is_hide)) {
              if (play.team_points && play.team_points instanceof Array) {
