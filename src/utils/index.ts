...
export function Combination(
  ...
  for (let i = n; i >= m; i--) {
    ...
    if (m > 1) {
      ...
      if (temp.length > 0) total.push(temp);
    }
  }
...
export function toFixedNumber(num: any, keep = 2): number {
  ...
  if (!Number.isNaN(tempValue)) {
    ...
    const result =
      Math.floor(tempValue * Math.pow(10, keep)) / Math.pow(10, keep);
  ...
  switch (value) {
    ...
    default:
      return `${i18n.global.t("No")}${
        getLocal("langConfig").id == 1 || getLocal("langConfig").id == 3
          ? NumberCn[value]
          : value
      }${lang || i18n.global.t("round")}`;
  }
}
...
export function getBetListStatus(v: any): any {
  if (+v.receive_status === 1) {
    ...
    if (+v.is_cancel === 1) {
      ...
      return { code: 4, color: "#4CD99D", name: i18n.global.t("not_clearing") };
